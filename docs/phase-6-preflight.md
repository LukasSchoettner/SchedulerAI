...
| Login works and authenticated requests mostly work, but `GET /day-plans/me/{date}` returns 500. scheduling-service logs show SQLState `42703` and `column follow_up_status does not exist`. | The local scheduling database schema is older than the current `DayPlanItem` entity and is missing follow-up tracking columns. | Restart or recreate the scheduling-service container so the idempotent scheduling schema initializer can add the missing columns while preserving existing rows. Recreate local dev database volumes only if data loss is acceptable. |
